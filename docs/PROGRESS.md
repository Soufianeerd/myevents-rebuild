...
Mis à jour le 21 septembre 2026. Ce suivi ne remplace ni le cahier des charges ni le registre de décisions.
...
- Connecteur Neon opérationnel : branche de staging et configuration Auth relues. CLI Vercel 59.23.2 : session renouvelée, projet relié à GitHub ; déploiement Preview en cours.
...
- Origine d'auth Preview sécurisée et testée ; `.vercelignore` audité sur les fichiers réellement envoyés. **150 tests** réussis après cette adaptation (81 unitaires, autres suites inchangées).
- GitHub : commits `a049567` puis `d6982b9` envoyés sur `codex/neon-migration`. Le projet Vercel est relié au dépôt ; variables Neon stockées dans Preview seulement.
- Premier lancement Vercel classé production malgré `--target preview` : build arrêté par le verrou `VERCEL_ENV`, aucune version opérationnelle. Deuxième lancement bien en Preview, arrêté car l'origine Auth n'était pas fournie ; `APP_URL` explicite ajouté pour la relance Git.
...
- Vercel `myevents` sur Hobby avec une Preview active ; production toujours verrouillée. Supabase `myevents-staging` créé gratuitement à Paris avant le changement de cible, sans migration appliquée. Accès PostgreSQL source validé, sauvegarde privée disponible et Neon staging restauré. La reprise Auth et les tests connectés de bout en bout restent nécessaires avant validation complète du parcours connecté.
- Usage GSAP dans l'éditeur visuel à clarifier avant adoption ; le modèle d'animation peut avancer indépendamment du moteur.

## Prochain

**Confirmer le nouvel accès du compte existant**, effectuer son rattachement explicite, puis vérifier connexion/reset, CRUD authentifié et persistance après redéploiement avant validation complète du parcours connecté. Compléter l'inventaire des configurations hors PostgreSQL. Aucun objet Storage source à transférer ; les fonctionnalités médias restent dans leurs sessions prévues. Développement suivant : R1 catalogue/capabilities local, R2 invitation/Studio/RSVP, R3 paiement/adapters test, R4 souvenirs, R5 print/admin, R6 acquisition/release. Détails et critères dans l'audit.
